...
def db_session():
    ...
    user_with_history = User(
        id=10, email="history@test.com", username="history_user",
        hashed_password="hashed", is_active=True,
    )
    new_user = User(
        id=11, email="newuser@test.com", username="new_user",
        hashed_password="hashed", is_active=True,
    )
    ...
    # Restaurants
    restaurants = [
        Restaurant(id=10, name="Sakura",       cuisine="japonaise",  city="Paris",    price_range=3, rating=4.9),
        Restaurant(id=11, name="Le Bistrot",   cuisine="française",  city="Paris",    price_range=2, rating=4.7),
        Restaurant(id=12, name="Bella Napoli", cuisine="italienne",  city="Lyon",     price_range=2, rating=4.6),
        Restaurant(id=13, name="Dragon",       cuisine="chinoise",   city="Paris",    price_range=1, rating=4.2),
        Restaurant(id=14, name="Spice Garden", cuisine="indienne",   city="Marseille",price_range=2, rating=4.8),
        Restaurant(id=15, name="Tacos",        cuisine="mexicaine",  city="Bordeaux", price_range=1, rating=3.9),
        Restaurant(id=16, name="Luxe",         cuisine="française",  city="Paris",    price_range=4, rating=4.5),
    ]
    ...
    # Réservations passées pour user_with_history
    reservations = [
        Reservation(id=10, user_id=10, restaurant_id=10,
                    date=date(2026, 5, 15), time=time(19, 0, 0), # <-- Vrais objets Python
                    status="completed", number_of_people=2),
        Reservation(id=11, user_id=10, restaurant_id=11,
                    date=date(2026, 5, 16), time=time(20, 0, 0), # <-- Vrais objets Python
                    status="completed", number_of_people=2),
    ]
...
def test_recommendations_with_history(client, headers_with_history):
    ...
    with patch("app.api.v1.endpoints.recommendations.get_cached_recommendations",
               return_value=None), \
...
def test_recommendations_cached(client, headers_with_history):
    ...
    cached_data = [
        {
            "id": 10, "name": "Sakura", "cuisine": "japonaise",
            "city": "Paris", "price_range": 3, "rating": 4.9,
            "score": 8.0, "score_reason": "cuisine préférée",
        }
    ]
